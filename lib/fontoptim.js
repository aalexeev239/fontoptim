...
FontOptim.prototype = {
	...
	getFontWeight: function(filename) {
		if (/-(thin|hairline)/i.test(filename)) return 100;
		if (/-(extra|ultra)-?light/i.test(filename)) return 200;
		if (/-light/i.test(filename)) return 300;
		if (/-medium/i.test(filename)) return 500;
		if (/-(semi|demi)-?bold/i.test(filename)) return 600;
		if (/-(extra|ultra)-?bold/i.test(filename)) return 800;
		if (/-bold/i.test(filename)) return 700;
		if (/-(black|heavy)/i.test(filename)) return 900;
		return 400;
	},

	getFontStyle: function(filename) {
		return (/italic/i.test(filename) || /it\./i.test(filename)) ? 'italic' : 'normal';
	},
